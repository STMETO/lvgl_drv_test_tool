...
static int sg90_probe(struct platform_device *pdev)
{
	...
	device_create(sg90_class, NULL, MKDEV(major, 0), NULL, "fxy_sg90"); 
...
static int __init sg90_init(void)
{
	...
	//注册设备
	major = register_chrdev(0, "fxy_sg90", &sg90_ops);

	//创建类，自动分配设备结点信息
	sg90_class = class_create(THIS_MODULE, "fxy_sg90_class");
	if (IS_ERR(sg90_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "fxy_sg90");
